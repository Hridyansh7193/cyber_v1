...
def passive_recon_node(state: NodeResult, config: BugHunterConfig) -> NodeResult:
    if not any(t.name == "node:passive_recon_node" for t in state.execution_state.task_queue):
        new_orch = start_task(state.orchestration_state, "passive_recon")
        new_orch = complete_task(new_orch, "passive_recon")
        return NodeResult(execution_state=state.execution_state, orchestration_state=new_orch)

    orch = start_task(state.orchestration_state, "passive_recon")
    new_exec = execute_node(
        current_exec=state.execution_state,
        config=config,
        capability=Capability.PASSIVE_RECON,
        wrapper_func=ReconWrapper.execute,
        wrapper_applier=apply_recon_wrapper_result,
        agent=analyze_recon,
        delta_applier=apply_recon_delta
    )
    orch = complete_task(orch, "passive_recon")
