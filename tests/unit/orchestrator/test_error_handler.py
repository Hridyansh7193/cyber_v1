...
from orchestrator.transitions import recon_transition
...
def test_handle_error_retry_exhausted():
    ...
    new_state = handle_task_error(state, "recon", "TimeoutError")
    assert new_state.task_status["recon"] == "FAILED"

def test_handle_error_cancellation():
    # Verify CANCELLED state
    state = OrchestrationState(task_status={"recon": "RUNNING"}, errors={})
    new_state = handle_task_error(state, "recon", "User cancelled")
...
def test_handle_error_graceful_continuation():
    ...
    state = OrchestrationState(task_status={"recon": "FAILED"}, errors={})
    graph_state: GraphState = {"execution_state": None, "orchestration_state": state}
    # recon_transition routes to js_node if COMPLETED or FAILED or CANCELLED
    # Let's ensure our transitions.py supports CANCELLED too, or FAILED is enough to continue
    assert recon_transition(graph_state) == "js_node"
