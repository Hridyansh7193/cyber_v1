    ...
    def run_scan_sync(self, domain: str, config: BugHunterConfig, metadata: dict = None, job_id: str = None, is_resume: bool = False) -> str:
        ...
        if final_state:
            ...
            try:
                ...
                errors = sum(len(log.wrapper_errors) + len(log.parser_errors) for log in final_state.logs) if hasattr(final_state, 'logs') and final_state.logs else 0
                warnings = 0
                findings_count = len(final_state.findings) if hasattr(final_state, 'findings') and final_state.findings else 0
                
                quality_score = 100.0 - (errors * 5.0) - (warnings * 2.0) + findings_count
