...
def track_scan_progress(scan_service, job_id: str):
                    ...
                    if dead_thread_count >= _DEAD_THREAD_GRACE_POLLS:
                        ...
                        if hasattr(scan_service, "_registry") and state not in (
                            ...
                            if job and job.status not in (
                                ...
                                logger.error("[PROGRESS] Forced job status to FAILED")
                        break
                else:
                    # Thread is alive, reset counter
                    if dead_thread_count > 0:
                        logger.debug("[PROGRESS] Worker thread is alive again, resetting dead count")
                    dead_thread_count = 0
    ...
    if worker_thread is not None:
        ...
        logger.info("[PROGRESS] Worker thread finished")
    else:
